...
clust_pix_list_full_file = storm_exp_directory + "pix_list_full.csv"
        ...
        if (np.all((stormtiff_image_array[j-1:j+2,i-1:i+2] == 0))):
            ...
            # for m, n in itertools.product(range(j-2, j+3), range(i-2, i+3)):    
                ...
                if ((m, n) in itertools.product(range(tile_start_pix_y, tile_start_pix_y+tile_size), range(tile_start_pix_x, tile_start_pix_x+tile_size))):
                    ...
                    # If tile pixels are in pixel list then get number of localizations from locs-dictionary dataframe else set them to zero.
                    if tup_nn in clstr_pix_list:
                        num += len(df[(df["y_floor"] == tup_nn[0]) & (df["x_floor"] == tup_nn[1])])
                    else:
                        pass
...
# # Save coefficients of the fit to file.
# with open(plots_directory + "linear_fit_uint8.csv","a") as f_out:
    # f_out.write("With linear fit for tiles in {} we have, # of localizations = {}*signal intensity + {},  \n" .format("750_ROIs_to_csv_img1_trunc.csv", coef1[0], coef1[1]))
    # f_out.write("With quadratic fit for for tiles in {} we have, # of localizations = {}*signal intensity**2 + {}*signal intensity + {},  \n" .format("750_ROIs_to_csv_img1_trunc.csv", coef2[0], coef2[1], coef2[2]))    

# # Save coefficients of the fit to file.
# with open(plots_directory + "linear_fit_8nn_average.csv","a") as f_out:
    # f_out.write("With linear fit for tiles in {} we have, # of localizations = {}*signal intensity + {},  \n" .format("750_ROIs_to_csv_img1_trunc.csv", coef1[0], coef1[1]))
    # f_out.write("With quadratic fit for for tiles in {} we have, # of localizations = {}*signal intensity**2 + {}*signal intensity + {},  \n" .format("750_ROIs_to_csv_img1_trunc.csv", coef2[0], coef2[1], coef2[2]))    
