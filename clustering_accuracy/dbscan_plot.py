...
if cluster_3d:
    ...
    for i in range(seq_start, seq_end+1):
        ...
        db_out_file_name = dbscan_seq_directory + "dbscan_out_tile_" + str(i) + ".data"            
    ...
    # for file in files[16:30]:    
        ...
        if tile_num not in tile_list_no_locs2d:        
            ...
            for k, col in zip(unique_labels, colors):
                ...
                # For plotting all class member points without noise points.
                xy = locs_2d[class_member_mask & ~noise_mask]             
            ...
            # plt.gca().xaxis.tick_top()
            # plt.gca().set_aspect('equal', adjustable='box')
            plt.title("Tile: {}, # of clusters: {}, molecule_list" .format(tile_num, n_clusters_))        
            # plt.title("Estimated number of clusters: %d" % n_clusters_)
            file_name = "dbscan_out_tile_{}_2d_core.jpg" .format(tile_num)
