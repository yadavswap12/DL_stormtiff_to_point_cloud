...
data_directory = expfolder + "make_data\\testing_data\\"
...
# data_directory_str = "chenghang_list_" + "tilesize_" + str(tile_size) + roi + "_original\\" 
...
locs2d_directory = accuracy_experiment_directory + "locs2d_mol_list\\"

# Remove previously present files.
files = glob.glob(locs2d_directory + "*")
for f in files:
    os.remove(f)
...
for img_num in df["Num_image"].unique():
    ...
    for idx in img_df.index:
        ...
        num_loc_file = num_locs_directory + "storm_num_locs_tile_" + str(tile_id) + ".data"        
        ...
        for j, i in itertools.product(range(0, tile_size), range(0, tile_size)):
            ...
            for num in range(round(num_locs)):
                
                num_loc_pixel.append(np.array([random.randint((j+tile_start_pix_y)*10**fl_pt, (j+tile_start_pix_y+1)*10**fl_pt)/10**fl_pt, random.randint((i+tile_start_pix_x)*10**fl_pt, (i+tile_start_pix_x+1)*10**fl_pt)/10**fl_pt]))
                num_loc_cart.append(np.array([nm_per_pixel_y*random.randint((j+tile_start_pix_y)*10**fl_pt, (j+tile_start_pix_y+1)*10**fl_pt)/10**fl_pt, nm_per_pixel_x*random.randint((i+tile_start_pix_x)*10**fl_pt, (i+tile_start_pix_x+1)*10**fl_pt)/10**fl_pt]))            
            
            # Add the pixel list to final localizations list.
            loc2d_pixel_list.extend(num_loc_pixel)
            loc2d_cart_list.extend(num_loc_cart)
            
            counter += 1
            
        # Print if locs_3d list is empty.
        if not loc2d_pixel_list:
            # print("locs_rd list for tile {} is empty." .format(tile_id))
            tile_list_no_locs2d.append(tile_id)

        # Convert list to numpy array.
        locs2d_pixel_arr = np.array(loc2d_pixel_list)
        locs2d_cart_arr = np.array(loc2d_cart_list)        

        # locs2d_pred_file = locs_pred_directory + "tile_" + str(img_num) + str(tile_count+1) + ".data"
        locs2d_pixel_file = locs2d_directory + "locs2d_molecule_list_pixel_tile_" + str(tile_id) + ".data"
        locs2d_cart_file = locs2d_directory + "locs2d_molecule_list_cart_tile_" + str(tile_id) + ".data"                    
